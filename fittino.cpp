...
void FillFixedParameters()
{
   ...
   if (!FindInFitted("Xtop") && !FindInFixed("Xtop") && !FindInUniversality("Xtop") && !FindInRandomPar("Xtop")) {
      tmpValue.name = "Xtop";
      tmpValue.value = 500.;
      tmpValue.error = -1.;
      tmpValue.bound_low = -1E+6;
   ...
   if (!FindInFitted("Xbottom") && !FindInFixed("Xbottom") && !FindInUniversality("Xbottom") && !FindInRandomPar("Xbottom")) {  
   ...
   void Fittino::markovChain ()
   {
	...
	system ("rm ./markovInterfaceFile.txt");
	 ...
	 seed = yyRandomGeneratorSeed;
      ...
      // ===================== start of preliminary tuning =======================
      if( yyPreliminaryScan )
	{
	  cout << yyDashedLine << endl;
	  cout << "Preliminary tuning of proposal width" << endl;
	  ...
	  int maxStep = 1000;
	  if( yyPrelScanIterations == 0 ) maxStep = 1000;
	  else maxStep = yyPrelScanIterations;
	  ...
	  for (unsigned int iVariable = 0; iVariable < x.size(); iVariable++) 
	  {
	    ...
	    int numChain = 0;
		 ...
		 while( step < maxStep )
		   {
		     cout << " ==== Individual tuning == STEP " << step << " ====="<< endl;
			 ...
			 cout << "step" << step << " --->IT Variable  = " << iVariable << endl;
			 cout << "step" << step << " --->IT Markov Chain success rate  = (" << successes <<")   "<<  successRate << endl;
			 cout << "step" << step << " --->IT Former width = "<<  vm[iVariable] << endl;
			 ...
			 cout << "step" << step << " --->IT New width ("<<  xNames[iVariable] <<") =  "<<  vm[iVariable] << endl;
		     }
		   }
	       }
	  }
