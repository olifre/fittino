				...
				int fixComb = 3, string link = "" ) 
{
  ...
  for (unsigned int fVariable = 0; fVariable < variables.size(); fVariable++)
      ...
      for (unsigned int sVariable = fVariable+1; sVariable < variables.size(); sVariable++)
	{
	  ...
	  TH2D *thisHist=0; // hist for 1D 68 % CL contour
	  TH2D *thisHist2=0; // hist for 2D 95 % CL contour
	  ...
	  // Frequentist Interpretation
		...
		if( variables[sVariable]=="af_direct" ){
		  sVarMin = 1e-12;
		  sVarMax = 1e-6;
		}
		if( variables[fVariable]=="af_direct" ){
		  fVarMin = 1e-12;
		  fVarMax = 1e-6;
		}
		
		if( variables[sVariable]=="O_massNeutralino1_nofit" ){
		  sVarMin = 30;
		  sVarMax = 1000;
		}
		if( variables[fVariable]=="O_massNeutralino1_nofit" ){
		...
		fVarMax = TMath::Log10(fVarMax);
	      ...
	      cout << "sVarMin sVarMax = " << sVarMin << "  " << sVarMax<< " fVarMin fVarMax = " << fVarMin << "  " << fVarMax << endl;

	      if (variables[sVariable] != "P_A0" && !doublelogplot && variables[sVariable] != "P_M0H" && variables[sVariable] != "P_M0Hu" && variables[sVariable] != "P_M0Hd") {
		if (sVarMin < 0) {
		  sVarMin = -2*sVarMax/(nBins-2);
		}
	      }

	      if (variables[fVariable] != "P_A0" && !doublelogplot && variables[fVariable] != "P_M0H" && variables[sVariable] != "P_M0Hu" && variables[sVariable] != "P_M0Hd") {
		if (fVarMin < 0) {
	  ...
	  else if (!strcmp(variables[sVariable].c_str(), "P_M0Hu")) {
            thisHist->SetXTitle("M_{0,Hu} (GeV)");
            emptyhist->SetXTitle("M_{0,Hu} (GeV)");
          }
	  else if (!strcmp(variables[sVariable].c_str(), "P_M0Hd")) {
            thisHist->SetXTitle("M_{0,Hd} (GeV)");
            emptyhist->SetXTitle("M_{0,Hd} (GeV)");
          }
