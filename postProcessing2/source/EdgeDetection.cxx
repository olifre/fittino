...
int main (int argc, char** argv){
  ...
  const TString inputFileName = argv[1];
    ...
    while (TKey* key = (TKey*) next())
    {
                ...
                for (Int_t iBinX = 0; iBinX < histImageHighContrast->GetNbinsX(); ++iBinX)
                    for (Int_t iBinY = 0; iBinY < histImageHighContrast->GetNbinsY(); ++iBinY)
                    {
                        histEdge->SetBinContent(
                                                  iBinX, iBinY,
                                                  TMath::Abs(histImageHighContrast->GetBinContent(iBinX, iBinY)
                                                - histImageHighContrast->GetBinContent(iBinX + 1, iBinY + 1))
						  + TMath::Abs(histImageHighContrast->GetBinContent(iBinX + 1, iBinY)
	    ...
	    if (drawSpline)
	      spline.Draw("SAME");
            ...
            canvas->SaveAs("Result" + (TString)histImageOriginal->GetName() + ".png", "RECREATE");
